...
- **`/create-design`** — ask for a Design System Name, run the CLI to add it, then interview through product visual-language decisions into the new spec-valid `DESIGN.md`. Reach for it before UI work when a project needs a new Design System.
- **`/add-design-template`** — create a reusable `.html` template inside a chosen Design System, using `/grill-with-docs` for the interview and that system's `DESIGN.md` as the visual authority.
- **`/archive-implementation`** — move non-setup implementation work into `.archive/` while preserving the AFK setup baseline and dot-prefixed project state. Reach for it when implementation inside a setup has become messy and you want to reset the active tree toward the initial setup.
