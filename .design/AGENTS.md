...
Before user-facing UI work, inspect `design-system/` for available Design Systems.

If `design-system/` is empty, ask the user which Design System to create or use. Do not invent a Design System name, visual direction, tokens, or style language.

If the task names a Design System, read its `README.md` and `DESIGN.md` before implementation. If multiple Design Systems exist and the task does not name one, ask the user which one to use.

Treat Design System files as user-authored product decisions. Do not overwrite them unless the user explicitly asks.

Use `afk-research design add "<Design System Name>"` to create a neutral scaffold for a new Design System.

## Creating Design Systems

Create each Design System under `design-system/<design-system-slug>/` using the scaffold files `README.md`, `DESIGN.md`, `design_tokens.json`, and `tailwind.config.js`.

Preserve the user-provided Design System Name in human-facing text. Derive the slug only for storage paths.

Keep new scaffolds neutral unless the user provides product-specific visual direction. Do not invent a brand personality, palette, font stack, component styling, or token values.

DESIGN.md is the source of truth. It should be self-contained, with optional YAML frontmatter for machine-readable tokens and markdown body sections for human-readable rationale and guidance.

When authoring `DESIGN.md`, use YAML frontmatter token groups for colors, typography, rounded, spacing, and components. Token values are normative; prose explains how to apply them.

Keep `design_tokens.json` and `tailwind.config.js` aligned with the tokens in `DESIGN.md`; they should not introduce conflicting design decisions.

## DESIGN.md Format Reference

The following DESIGN.md format reference is intentionally copied into this AGENTS.md template so every generated Design Workspace carries the full design-system authoring guidance.
